...
hard_svc= svm.SVC(kernel='linear', C=30000).fit(X, y)  # 硬间隔
soft_svc= svm.SVC(kernel='linear', C=0.00001).fit(X, y)  # 软间隔
# 创建网格，以绘制图像
...
# 图的标题
titles = ['SVC-inear kernel',
          'LinearSVC (linear kernel)',
          'SVC-RBF kernel',
          'SVC-polynomial\n(degree = 3)kernel',
          'Hard SVC-linear kernel\n(C=30000)',
          'Soft SVC-linear kernel\n(C=1.0)']

for i, clf in enumerate((svc, lin_svc, rbf_svc, poly_svc,hard_svc,soft_svc)):
    # 绘出决策边界，不同的区域分配不同的颜色
    plt.subplot(2, 3, i + 1)  # 创建一个2行2列的图，并以第i个图为当前图
    plt.subplots_adjust(wspace=0.6, hspace=0.6)  # 设置子图间隔
